...
/**
 * Extrae precio de una página de producto de Naldo.
 * Estrategias en orden de confiabilidad:
 *   1. JSON-LD schema.org (offers.price)
 *   2. VTEX __STATE__ JSON embebido
 *   3. Regex sobre clase prices-main-price / sellingPrice
 *
 * @param {string} url - URL del producto en naldo.com.ar
 * @returns {string|null} Precio formateado (ej: "$249.499") o null si falla
 */
async function scrapeNaldo(url) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 5000);

  try {
    logInfo('scraper-naldo', `Scraping: ${url}`);
    const res = await fetch(url, { headers: HEADERS, signal: controller.signal });

    if (!res.ok) {
      logWarn('scraper-naldo', `HTTP ${res.status} para ${url}`);
      return null;
    }
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (price) {
            ...
            if (formateado) {
              logInfo('scraper-naldo', `Precio por JSON-LD: ${formateado}`);
              return formateado;
            }
          }
        }
    ...
    logWarn('scraper-naldo', `No se encontró precio en ${url}`);
    return null;
  } catch (err) {
    if (err.name === 'AbortError') {
      logWarn('scraper-naldo', `Timeout (5s) para ${url}`);
    } else {
      logError('scraper-naldo', `Error: ${err.message}`);
    }
    return null;
  } finally {
